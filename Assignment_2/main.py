...
# Load the dataset
data = pd.read_csv("Assignment_2/steel.csv")
...
# Tune the SVR model
svr_param_grid = {'kernel': ['linear', 'rbf', 'poly'], 'epsilon': [0.1, 0.2, 0.5, 1.0, 5.0]}
...
svr_default_scores = [-svr_score_mse.mean(), -svr_score_mae.mean(), -svr_score_mape.mean()]
svr_tuned_scores = [svr_grid.best_score_, -cross_val_score(svr_grid.best_estimator_, x, y, cv=kf, scoring='neg_mean_absolute_error').mean(), -cross_val_score(svr_grid.best_estimator_, x, y, cv=kf, scoring='neg_mean_absolute_percentage_error').mean()]

rf_default_scores = [-rf_score_mse.mean(), -rf_score_mae.mean(), -rf_score_mape.mean()]
rf_tuned_scores = [rf_grid.best_score_, -cross_val_score(rf_grid.best_estimator_, x, y, cv=kf, scoring='neg_mean_absolute_error').mean(), -cross_val_score(rf_grid.best_estimator_, x, y, cv=kf, scoring='neg_mean_absolute_percentage_error').mean()]

fig, axes = plt.subplots(1, 2, figsize=(14, 6))

axes[0].plot(metrics, svr_default_scores, label='Default SVR', marker='o')
axes[0].plot(metrics, svr_tuned_scores, label='Tuned SVR', marker='o')
axes[0].set_title('SVR Model Performance')
axes[0].legend()

axes[1].plot(metrics, rf_default_scores, label='Default RF', marker='o')
axes[1].plot(metrics, rf_tuned_scores, label='Tuned RF', marker='o')
axes[1].set_title('Random Forest Model Performance')
axes[1].legend()
